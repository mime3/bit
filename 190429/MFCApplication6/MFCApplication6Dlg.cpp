...
BEGIN_MESSAGE_MAP(CMFCApplication6Dlg, CDialogEx)
	...
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_BUTTON_INPUT, &CMFCApplication6Dlg::OnBnClickedButtonInput)
	ON_BN_CLICKED(IDC_BUTTON_DEL, &CMFCApplication6Dlg::OnBnClickedButtonDel)
	ON_BN_CLICKED(IDC_BUTTON_INIT, &CMFCApplication6Dlg::OnBnClickedButtonInit)
	ON_BN_CLICKED(IDC_BUTTON_EXIT, &CMFCApplication6Dlg::OnBnClickedButtonExit)
END_MESSAGE_MAP()
...
void CMFCApplication6Dlg::OnBnClickedButtonInput()
{
	...
	item.iSubItem = 0;
	wsprintf(szText, _T("%s"), strNum/*.GetBuffer()*/);
	item.pszText = (LPTSTR)szText;
	listView.InsertItem(&item);

	item.iSubItem = 1;
	wsprintf(szText, _T("%s"), strName/*.GetBuffer()*/);
	item.pszText = (LPTSTR)szText;
	listView.SetItem(&item);

	item.iSubItem = 2;
	wsprintf(szText, _T("%s"), strCell/*.GetBuffer()*/);
	item.pszText = (LPTSTR)szText;
	listView.SetItem(&item);
...
void CMFCApplication6Dlg::OnBnClickedButtonInit()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	Init();
}
...
void CMFCApplication6Dlg::OnBnClickedButtonExit()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	OnOK();
}
