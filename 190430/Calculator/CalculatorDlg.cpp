...
BEGIN_MESSAGE_MAP(CCalculatorDlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_BUTTON_CAL, &CCalculatorDlg::OnBnClickedButtonCal)
	ON_BN_CLICKED(IDC_INIT, &CCalculatorDlg::OnBnClickedInit)
END_MESSAGE_MAP()
...
void CCalculatorDlg::OnBnClickedButtonCal()
{
	...
	HMODULE hModule = ::LoadLibrary(_T("CalculatorDLL.dll"));
	if (hModule != NULL)
	{
		int(WINAPI * pfnCalculator)(int, int, WCHAR) = nullptr;
		pfnCalculator = (int(WINAPI*)(int, int, WCHAR))::GetProcAddress(hModule, "Calculator");
		result = pfnCalculator(num1, num2, operand);
		::FreeLibrary(hModule);
	}
	UpdateData(FALSE);
}
...
void CCalculatorDlg::OnBnClickedInit()
{
	result = num1 = num2 = 0;
	UpdateData(FALSE);
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
