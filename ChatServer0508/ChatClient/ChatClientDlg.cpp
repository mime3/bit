...
{
	CDialogEx::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_EDIT_NICKNAME, EDIT_NICKNAME);
}
...
void CChatClientDlg::OnBnClickedOk()
{
	...
	CString nick;
	EDIT_NICKNAME.GetWindowTextW(nick);
	StringCchCopy(g_nickName, 20, nick.GetString());

	//CString IP;
	//EDIT_IP.GetWindowTextW(IP);
	//StringCchCopy(g_IP, 20, IP.GetString());
