...
BOOL CMFCSocketDlg::OnInitDialog()
{
	...
	if (m_listenSocket.Create(20000, SOCK_STREAM))
	{
		...
		{
			AfxMessageBox(_T("ERROR : Listen Error"));
		}
	}
	else
		AfxMessageBox(_T("ERROR : Failed to create listen socket"));
...
void CMFCSocketDlg::OnDestroy()
{
	...
	POSITION pos;
	pos = m_listenSocket.m_ptrClientSokcet.GetHeadPosition();
	CClientSocket* client = NULL;

	while (pos != NULL)
	{
		client = (CClientSocket*)m_listenSocket.m_ptrClientSokcet.GetNext(pos);
		if (client != NULL)
		{
			client->ShutDown();
			client->Close();
