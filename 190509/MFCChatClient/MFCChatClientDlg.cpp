...
void CMFCChatClientDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_EDIT_CHAT, m_chatEdit);
}
...
BOOL CMFCChatClientDlg::OnInitDialog()
{
	...
	if (m_connectSocket.Connect(_T("127.0.0.1"), 20000) == FALSE)
	{
...
void CMFCChatClientDlg::OnBnClickedButtonSend()
{
	...
	UpdateData();
	m_connectSocket.Send((LPVOID)(LPCTSTR)m_chatEdit, m_chatEdit.GetLength() * 2);
	m_chatEdit = _T("");
	UpdateData(FALSE);
}
